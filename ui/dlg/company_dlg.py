...
class CompanyDialog(QtWidgets.QDialog):
    def __init__(self, *, app_data, loaded_company=None):
        ...
        """ set title """
        dialog_title = (
            f"Firma ({loaded_company.name}) bearbeiten..."
            if loaded_company
            else "Neue Firma"
        )
    ...
    def update_ui(self):
        ...
        sel_c_p_first_name = (
            self.sel_contact_person.first_name if self.sel_contact_person else "-"
        )
        sel_c_p_last_name = (
            self.sel_contact_person.last_name if self.sel_contact_person else "-"
        )
        args = self.get_input()
        self.set_labels(
            budget_w_VAT=args["budget"] * (1 + self.vat),
    ...
    def set_button_actions(self):
        ...
        self.pushButton_delete.clicked.connect(
            lambda: helper.delete(self, self.loaded_company)
        )
    ...
    def set_input(
        self, *, _uid=None, name="", service="", service_type="", budget=0, **kwargs
    ):
        """data"""
