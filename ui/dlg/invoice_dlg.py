...
class InvoiceDialog(QtWidgets.QDialog):
    def __init__(self, *, app_data, sel_job=None, loaded_invoice=None):
        ...
        """ set title """
        dialog_title = (
            f"Rechnung ({loaded_invoice.id}) bearbeiten..."
            if loaded_invoice
            else "Neue Rechnung"
        )
    ...
    def set_button_actions(self):
        """date buttons"""
        self.pushButton_invoice_date_set_today.clicked.connect(
            lambda: self.dateEdit_invoice_date.setDate(QDate.currentDate())
        )
        self.pushButton_inbox_date_set_today.clicked.connect(
            lambda: self.dateEdit_inbox_date.setDate(QDate.currentDate())
        )
        self.pushButton_checked_date_set_today.clicked.connect(
            lambda: self.dateEdit_checked_date.setDate(QDate.currentDate())
        )
        self.pushButton_due_date_set_today.clicked.connect(
            lambda: self.dateEdit_due_date.setDate(QDate.currentDate())
        )
        self.pushButton_due_date_discount_set_today.clicked.connect(
            lambda: self.dateEdit_due_date_discount.setDate(QDate.currentDate())
        )
        ...
        self.pushButton_delete.clicked.connect(
            lambda: helper.delete(self, self.loaded_invoice)
        )

    def set_check_box_actions(self):
        """checkbox enable disable"""
        self.checkBox_inbox.toggled.connect(self.dateEdit_inbox_date.setEnabled)
        self.checkBox_inbox.toggled.connect(
            self.pushButton_inbox_date_set_today.setEnabled
        )
        self.checkBox_checked.toggled.connect(self.dateEdit_checked_date.setEnabled)
        self.checkBox_checked.toggled.connect(
            self.pushButton_checked_date_set_today.setEnabled
        )
        self.checkBox_w_VAT.toggled.connect(self.doubleSpinBox_VAT.setEnabled)
        self.checkBox_sd_absolute.toggled.connect(
            self.doubleSpinBox_safety_deposit_absolute.setEnabled
        )
        self.checkBox_sd_absolute.toggled.connect(
            self.doubleSpinBox_safety_deposit.setDisabled
        )
        self.checkBox_discount.toggled.connect(
            self.dateEdit_due_date_discount.setEnabled
        )
        self.checkBox_discount.toggled.connect(
            self.pushButton_due_date_discount_set_today.setEnabled
        )
        self.checkBox_discount.toggled.connect(
            self.pushButton_due_date_discount_set_today.setEnabled
        )
    ...
    def set_spin_box_actions(self):
        ...
        self.doubleSpinBox_reduction_insurance_costs.valueChanged.connect(
            self.update_ui
        )
    ...
    def update_prev_invoices(self):
        args = self.get_input()
        invoice_uid = (
            self.loaded_invoice.uid if self.loaded_invoice else self.tmp_invoice.uid
        )
        invoice_created_date = (
            self.loaded_invoice.uid.created_date
            if self.loaded_invoice
            else self.tmp_invoice.uid.created_date
        )
        self.prev_invoices = self.app_data.project.get_prev_invoices(
    ...
    def set_labels(
        ...
        *,
        ...
        self.label_approved_amount_a_discount_amount.setText(
            amount_str(
                approved_amount_a_discount_amount, self.app_data.project.get_currency()
            )
        )
        """ safety deposit """
    ...
    def set_input(
        ...
        *,
        ...
        inbox_date=QDate.currentDate(),
        checked_date=QDate.currentDate(),
        company=None,
        job=None,
        amount=0,
        verified_amount=0,
        ...
        safety_deposit=0,
        _safety_deposit_amount=None,
        discount=0,
        due_date=QDate.currentDate(),
        due_date_discount=QDate.currentDate(),
        **kwargs,
    ):
        ...
        self.lineEdit_amount_2.setText(
            f"{int(rnd(amount-int(amount))*100):02d}"
            if amount - int(amount) > 0
            else ""
        )
        self.lineEdit_verified_amount_1.setText(
            str(int(verified_amount)) if int(verified_amount) > 0 else ""
        )
        self.lineEdit_verified_amount_2.setText(
            f"{int(rnd(verified_amount-int(verified_amount))*100):02d}"
            if verified_amount - int(verified_amount) > 0
            else ""
        )
        """ reductions """
        self.doubleSpinBox_rebate.setValue(rebate * 100)
        self.doubleSpinBox_reduction_insurance_costs.setValue(
            reduction_insurance_costs * 100
        )
    ...
    def get_input(self):
        args = {
            ...
            "inbox_date": self.dateEdit_inbox_date.date()
            if self.checkBox_inbox.isChecked()
            else None,
            "checked_date": self.dateEdit_checked_date.date()
            if self.checkBox_checked.isChecked()
            else None,
            "amount": two_inputs_to_float(
                self.lineEdit_amount_1, self.lineEdit_amount_2
            ),
            "verified_amount": two_inputs_to_float(
                self.lineEdit_verified_amount_1, self.lineEdit_verified_amount_2
            ),
            "rebate": self.doubleSpinBox_rebate.value() / 100,
            "reduction_insurance_costs": self.doubleSpinBox_reduction_insurance_costs.value()
            / 100,
            "reduction_usage_costs": self.doubleSpinBox_reduction_usage_costs.value()
            / 100,
            "reduce_prev_invoices": self.checkBox_reduce_prev_invoices.isChecked(),
            "VAT": self.doubleSpinBox_VAT.value() / 100
            if self.checkBox_w_VAT.isChecked()
            else 0,
            "safety_deposit": self.doubleSpinBox_safety_deposit.value() / 100
            if not (self.checkBox_sd_absolute.isChecked())
            else 0,
            "safety_deposit_amount": self.doubleSpinBox_safety_deposit_absolute.value()
            if self.checkBox_sd_absolute.isChecked()
            else None,
            "discount": self.doubleSpinBox_discount.value() / 100
            if self.checkBox_discount.isChecked()
            else 0,
            "due_date": self.dateEdit_due_date.date(),
            "due_date_discount": self.dateEdit_due_date_discount.date()
            if self.checkBox_discount.isChecked()
            else None,
        }
        return args
