...
def open_inventory_item_dialog(app_data, loaded_inventory_item=None):
    inventory_item_dlg = dlg.InventoryItemDialog(
        app_data=app_data, loaded_inventory_item=loaded_inventory_item
    )
...
def open_invoice_dialog(app_data, sel_job=None, loaded_invoice=None):
    invoice_dlg = dlg.InvoiceDialog(
        app_data=app_data, sel_job=sel_job, loaded_invoice=loaded_invoice
    )
...
def open_job_dialog(app_data, sel_company=None, loaded_job=None):
    job_dlg = dlg.JobDialog(
        app_data=app_data, sel_company=sel_company, loaded_job=loaded_job
    )
...
def open_cost_group_dialog(app_data, loaded_cost_group=None):
    cost_group_dlg = dlg.CostGroupDialog(
        app_data=app_data, loaded_cost_group=loaded_cost_group
    )
...
def open_save_curr_project_prompt(dialog):
    reply = QtWidgets.QMessageBox.question(
        dialog,
        "Projekt geöffnet...",
        "Ein Projekt ist bereits geöffnet. Möchten Sie speichern bevor ein neues geladen wird?",
