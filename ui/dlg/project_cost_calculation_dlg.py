...
class ProjectCostCalculationDialog(QtWidgets.QDialog):
    ...
    def initialize_ui(self):
        ...
        self.treeWidget_inventory.setHeaderLabels(
            [self.app_data.titles[col] for col in tree_widget_cols]
        )
    ...
    def activate_reset_filter_button(self):
        if (
            self.comboBox_filter_cost_group.currentData()
    ...
    def set_widget_actions(self):
        self.treeWidget_inventory.itemDoubleClicked.connect(
            self.double_click_inventory_item_tree
        )
        self.treeWidget_inventory.itemClicked.connect(
            self.activate_inventory_item_buttons
        )

    def set_button_actions(self):
        self.pushButton_reset_filter.clicked.connect(self.button_reset_filter)
        self.pushButton_add_inventory_item.clicked.connect(
            self.button_new_inventory_item
        )
        self.pushButton_edit_inventory_item.clicked.connect(
            self.button_edit_inventory_item
        )
        self.pushButton_copy_inventory_item.clicked.connect(
            self.button_copy_inventory_item
        )
        self.pushButton_remove_inventory_item.clicked.connect(
            self.button_remove_inventory_item
        )
        """ delete button """
        self.pushButton_delete.clicked.connect(
            lambda: helper.delete(self, self.loaded_pcc)
        )

    def set_combo_box_actions(self):
        self.comboBox_filter_cost_group.currentIndexChanged.connect(
            self.setup_combo_box_filter_by_trade
        )
    ...
    def button_edit_inventory_item(self):
        inventory_item = self.treeWidget_inventory.currentItem().data(
    ...
    def get_inventory(self):
        ...
        if trade:
            ...
            inventory = [
                ...
                if item.cost_group is cost_group
                or item.cost_group.is_sub_group_of(cost_group)
            ]
